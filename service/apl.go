...
func exportAplList(aw *app.Wrapper, aplList []model.AplListVO) {
	var dto []util.AplXlsxRow
	for _, item := range aplList {
		...
		dto = append(dto, util.AplXlsxRow{
			...
			Status:      statusStr,
			Score:       item.Score,
			ScoreExtra:  item.ScoreExtra,
			Note:        item.Note,
		})
	}
	byt, err := util.ExportAplList(dto)
	if err != nil {
		aw.Error(err.Error())
		return
	}
...
func (receiver AplService) JudgeApl(c *gin.Context) {
	...
	aw.Success(req.ApplicationJudge)
}
...
func (receiver AplService) ListUserApls(c *gin.Context) {
	...
	aw.Success(aplModel.FindAplsByUserID(req.UserID))
}
